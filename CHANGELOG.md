...
- `FileIndex.QueryUsnJournalSettings` and the `UsnJournalSettings` record report a watched volume's USN journal sizing (`MaximumSize`, `AllocationDelta`, `IsBelowRecommended`) without elevation, via `FSCTL_QUERY_USN_JOURNAL` against a backup-semantics handle on the volume root ([MFTLib#141](https://gitea.fleet.sticktoitive.net/schoen/MFTLib/issues/141))
- `UsnJournalRecommendations` carries the recommended journal sizing (128 MB maximum, 16 MB allocation delta) so consumers never hard-code the numbers ([MFTLib#141](https://gitea.fleet.sticktoitive.net/schoen/MFTLib/issues/141))
