...
- `DriveStatus.CheckpointLoss` and the `JournalCheckpointLoss` record report, without elevation, that a drive was cold-scanned because its cached block's USN journal checkpoint was no longer in the journal: the checkpoint USN, the journal's `FirstUsn` and `NextUsn` when the loss was detected, its `AllocationDelta` and `MaximumSize`, how far behind the checkpoint was in bytes, and the journal maximum size that would have retained it. USNs are byte offsets, so all of it is exact integer arithmetic over numbers read off the volume ([MFTLib#217](https://gitea.fleet.sticktoitive.net/schoen/MFTLib/issues/217))
- `JournalCheckpointLossCause` separates the two causes MFTLib can tell apart: `CheckpointTrimmed`, where the journal is the one the checkpoint came from and a larger journal would have kept it, and `JournalRecreated`, where the journal carries a different id so no size would have helped and none is suggested. Opening a drive now reads the live journal before adopting a cached block, so an unresumable checkpoint becomes one cold scan with a reason attached instead of a warm start whose watch dies on its first read ([MFTLib#217](https://gitea.fleet.sticktoitive.net/schoen/MFTLib/issues/217))
