...
{
    readonly Func<string, UsnJournalCursor> _queryCursor;
    readonly Func<string, ScanRecord[]> _scanDrive;
    readonly Func<string, UsnJournalCursor, (UsnJournalEntry[] Entries, UsnJournalCursor Updated)> _readJournal;
    readonly Func<string, UsnJournalCursor, CancellationToken,
        IAsyncEnumerable<(UsnJournalEntry[] Entries, UsnJournalCursor Cursor)>>? _watchDrive;

    public JournalBrokerHost(
        Func<string, UsnJournalCursor> queryCursor,
        Func<string, ScanRecord[]> scanDrive,
        Func<string, UsnJournalCursor, (UsnJournalEntry[], UsnJournalCursor)> readJournal,
        Func<string, UsnJournalCursor, CancellationToken,
            IAsyncEnumerable<(UsnJournalEntry[], UsnJournalCursor)>>? watchDrive = null)
    {
