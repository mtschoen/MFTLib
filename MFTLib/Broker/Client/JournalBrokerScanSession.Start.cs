...
public sealed partial class JournalBrokerScanSession
{
    ...
    ///     (profile, block destinations, keepFileNames, progress), and return a session parked on the result.
    ...
    // The public overload above delegates here with connectAsync set to
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var session = new JournalBrokerScanSession(client, drives, profile, EmptyCursors);
        try
        {
            ...
            if (!published)
            {
                // The broker died while the initial scan was in flight, so the session
                // never took the result and the DisposeAsync below has nothing to release
                // it by. Same ownership rule as a rescan whose publication is refused.
    ...
    ///     until the first <see cref="RescanAsync(BrokerScanOptions,CancellationToken)" />, so <see cref="LatestScan" /> is null until
    ///     then. Each rescan requires explicit options with block targets, profile, and keep-file names.
    /// </summary>
    ...
    ///     but with an explicit initial <paramref name="profile" />. Each later rescan uses
    ///     its explicit options, including keep-file names and block targets.
    /// </summary>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="launchBroker" /> or <paramref name="cursorsByDrive" /> is null.</exception>
    /// <exception cref="ArgumentException">Thrown when a key in <paramref name="cursorsByDrive" /> is not a valid drive letter.</exception>
    [SupportedOSPlatform("windows")]
        ...
        CancellationToken cancellationToken = default)
    {
        return StartFromCursorsAsync(
            cancellation => JournalBrokerClient.SpawnAndConnectAsync(launchBroker, cancellation),
            cursorsByDrive, profile, cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var client = await connectAsync(cancellationToken).ConfigureAwait(false);
        // Rescans that omit a drive list use the warm-start volumes.
        return new JournalBrokerScanSession(
            client, watchCursors.Keys.ToArray(), profile, watchCursors);
    }
