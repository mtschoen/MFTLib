...
public sealed partial class JournalBrokerClient : IAsyncDisposable
{
    ...
    {
        // Build per-drive MMFs and the drivesSpec for the ArmAndScan frame.
        ...
        foreach (var drive in drives)
        {
            ...
            // Cold arm: journalId=0 nextUsn=0. The broker queries the real cursor.
            specTokens.Add(FormattableString.Invariant($"{letter}:0:0:{mmfName}"));
        }

        var drivesSpec = string.Join(",", specTokens);
        ...
        // Collect per-drive results until every drive has reported in.
        var remainingDrives = new HashSet<string>(
            drives.Select(NormalizeDriveLetter), StringComparer.OrdinalIgnoreCase);

        var allRecords = new List<ScanRecord>();
        var armedCursors = new Dictionary<string, UsnJournalCursor>(StringComparer.OrdinalIgnoreCase);
        var advancedCursors = new Dictionary<string, UsnJournalCursor>(StringComparer.OrdinalIgnoreCase);
        var catchUpEntries = new Dictionary<string, UsnJournalEntry[]>(StringComparer.OrdinalIgnoreCase);
        var errors = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

        while (remainingDrives.Count > 0)
        {
            ...
            switch (frame.Value.Kind)
            {
                case BrokerFrameKind.Cursor:
                    {
                        ...
                        armedCursors[drive] = frame.Value.Cursor;
                        break;
                    }
                ...
                case BrokerFrameKind.ScanReady:
                    {
                        ...
                        var mmfName = frame.Value.RequireMmfName();
                        ...
                        {
                            var records = _mmfReader.Read(mmfName, frame.Value.ByteLength);
                ...
                case BrokerFrameKind.JournalBatch:
                    {
                        var drive = frame.Value.RequireDrive();
                        advancedCursors[drive] = frame.Value.Cursor;
                        catchUpEntries[drive] = frame.Value.Entries;
                        // A drive is complete after Cursor + ScanReady + JournalBatch.
                        // (Error also completes a drive; handled below.)
                        remainingDrives.Remove(drive);
                        break;
                    }
