...
- **MFTLib** (C# Library) - Managed wrapper with P/Invoke interop. The `MFTLib.Index` namespace provides a substrate-neutral columnar block format and query engine; see `docs/index-format.md`.
    ...
    - **Watch start readiness**: `FileIndex.StartWatchingAsync` completes only once the session's
      ...
      source through `IIndexWatchSource.StartWatching(targets, reportStreamReady,
      teardownCancellationToken, cancellationToken)`, whose default forwards to the three-argument
      overload;
      ...
      `StopLiveWatchAsync` before releasing the stream. The source's stream stays claimed until
      then, and a new `StartWatching` on the same source waits for that teardown (bounded by its
      own token) instead of being rejected, so the next StartWatch frame is never sent ahead of the
      old watch's EndWatch.
      Both stop paths share one rule, watch generations (MFTLib issue 252). Every watch a
      `JournalBrokerClient` starts takes a fresh client-wide generation number that its StartWatch,
      EndWatch, and EndWatchAck frames all carry; the demux ends only on the EndWatchAck for its own
      generation and ignores any other, and the host stops only the generation an EndWatch names
      and stops the whole old generation before arming a StartWatch for a new one. There is no
      fixed acknowledgement timer: `StopLiveWatchAsync(cancellationToken)` waits for its
      generation's EndWatchAck or pipe EOF (a dead broker) bounded only by its token and client
      disposal. A stop cancelled before the ack throws `OperationCanceledException` after the
      client-side teardown, and one cancelled while another operation holds the arm-ordering gate
      defers that teardown to the next gate holder (`CompleteDeferredLiveWatchStopAsync`); either
      way the connection stays usable for the next watch with no reconnect or new UAC prompt. The
      EndWatch write is never cut off by the stop's token. `FileIndex.StopWatchingAsync(token)`
      cancels `WatchSession.TeardownToken`, which `BrokerIndexWatchSource.StopStreamAsync` passes to
      `StopLiveWatchAsync`; disposal and `StopWatchingAsync(CancellationToken.None)` leave it
      uncancelled, so they wait for the ack or EOF. A later claim on the source cancels an
      abandoned start's teardown token, so that start watches as soon as the abandoned send has
      finished. A teardown that fails for any other reason is recorded rather than thrown, so the
      retirement task never faults, and it fails every later start on that source with an
      `InvalidOperationException` wrapping it. Frames carrying generation 0, or laid out without a
      generation, are rejected as a client/broker build mismatch; there is no separate handshake.
