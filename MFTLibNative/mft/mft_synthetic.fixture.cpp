...
uint16_t WriteFixtureDataAttribute(uint8_t* record, uint16_t offset, const FixtureDataSpec& data) {
    ...
    uint64_t nonnegativeSize = data.size > 0 ? static_cast<uint64_t>(data.size) : 0;
    uint64_t allocatedSize = (nonnegativeSize + (clusterSize - 1ULL)) & ~(clusterSize - 1ULL);
