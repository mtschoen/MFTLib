...
bool AllocateParseBuffers(std::array<uint8_t*, 2>& buf, size_t bufSize, PathLookup& lookup, bool resolvePaths,
    ...
    if ((buf[0] == nullptr) || (buf[1] == nullptr)) {
        ...
        SetErrorMessage(state.result->errorMessage, L"Failed to allocate I/O buffers");
        return false;
    }
    ...
    state.result->entries =
        ShouldFailAlloc()
            ? nullptr
            : static_cast<MftFileEntry*>(malloc(static_cast<size_t>(state.capacity) * sizeof(MftFileEntry)));
    if (state.result->entries == nullptr) {
        mftlib::platform::big_free(buf[0], bufSize);
        mftlib::platform::big_free(buf[1], bufSize);
        if (resolvePaths) {
            lookup.cleanup();
        }
        SetErrorMessage(state.result->errorMessage, L"Failed to allocate entry array");
        return false;
    }
    return true;
}
...
bool MergeSlices(std::vector<SliceResult>& slices, unsigned actualThreads, ParseState& state) {
    for (unsigned ti = 0; ti < actualThreads; ti++) {
        ...
        if (state.usedCount + sliceCount > state.capacity) {
            uint64_t newCapacity = state.capacity;
            while (state.usedCount + sliceCount > newCapacity) {
                newCapacity *= 2;
            }
            auto* grown = ShouldFailAlloc()
                              ? nullptr
                              : static_cast<MftFileEntry*>(realloc(
                                    state.result->entries, static_cast<size_t>(newCapacity) * sizeof(MftFileEntry)));
            if (grown == nullptr) {
                SetErrorMessage(state.result->errorMessage, L"Failed to grow entry array");
                state.result->usedRecords = state.usedCount;
                return false;
            }
            state.result->entries = grown;
            state.capacity = newCapacity;
        }
        memcpy(state.result->entries + state.usedCount, slice.entries.data(),
               static_cast<size_t>(sliceCount) * sizeof(MftFileEntry));
        state.usedCount += sliceCount;
    }
    return true;
}
...
bool ParseChunkParallel(uint8_t* buffer, ChunkSpan chunk, unsigned numThreads, const FilterSpec& filter,
                        PathLookup* lookup, uint64_t totalRecords, ParseState& state) {
    ...
    for (unsigned ti = 0; ti < numThreads; ti++) {
        ...
        workers.emplace_back(
            [buffer, tStart, tEnd, ti, &slices, &threadFixupMs, filter, lookup, totalRecords, recordIndex]() {
                auto fStart = SteadyClock::now();
                FixupRange(buffer, SliceRange{tStart, tEnd});
                threadFixupMs[ti] = ElapsedMs(fStart, SteadyClock::now());
                ProcessRecordSlice(buffer, SliceRange{tStart, tEnd}, recordIndex, &slices[ti], filter, lookup,
                                   totalRecords);
            });
    }
...
void ResolveAllPaths(uint64_t totalRecords, PathLookup& lookup, unsigned numThreads, ParseState& state) {
    MftParseResult* result = state.result;
    uint64_t usedCount = state.usedCount;
...
bool ParseAllChunks(ReadChunkFn readChunk, void* readContext, std::array<uint8_t*, 2>& buf, unsigned numThreads,
                    const FilterSpec& filter, PathLookup* lookup, uint64_t totalRecords, ParseState& state) {
    ...
    while (currentChunkSize > 0) {
        ...
        if (numThreads > 1) {
            if (!ParseChunkParallel(buffer, ChunkSpan{recordIndex, currentChunkSize}, numThreads, filter, lookup,
                                    totalRecords, state)) {
                ioThread.join();
                return false;
            }
            ...
            ProcessRecordBatch(buffer, currentChunkSize, recordIndex, state.result, state.usedCount, state.capacity,
                               filter, lookup, totalRecords);
            state.parseMs += ElapsedMs(parseStart, SteadyClock::now());
            if (state.result->errorMessage[0] != L'\0') {
                ioThread.join();
                break;
            }
        }
...
MftParseResult* ParseMFTImpl(ReadChunkFn readChunk, void* readContext, uint64_t totalRecords, FilterSpec filter,
    ...
    ParseState state = {};
    state.result = result;
    state.capacity = std::max<uint64_t>((filter.text != nullptr) ? 1024 : totalRecords / 4, 1024);
    ...
    bool parsedOk = ParseAllChunks(readChunk, readContext, buf, numThreads, filter, resolvePaths ? &lookup : nullptr,
                                   totalRecords, state);
    ...
    if (resolvePaths && state.usedCount > 0) {
        ResolveAllPaths(totalRecords, lookup, numThreads, state);
        lookup.cleanup();
    }

    result->usedRecords = state.usedCount;
