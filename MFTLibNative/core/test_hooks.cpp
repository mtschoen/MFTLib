...
bool ShouldFailPlatformRead() {
    if (g_failPlatformReadCountdown <= 0) return false;
...
bool UsnIoInjectSuccess(void* outBuffer, unsigned long outBufferSize, unsigned long* bytesReturned) {
    if (g_usnIoCount <= 0) return false;
    ...
    if (data && copyLen) memcpy(outBuffer, data, copyLen);
    if (bytesReturned) *bytesReturned = copyLen;
    return true;
}

bool UsnIoShouldAbortOverlapped() {
    if (!g_usnOverlappedAbort) return false;
...
extern "C" {
    ...
    EXPORT void SetUsnIoSuccess(const uint8_t* data, uint32_t size) {
        if (g_usnIoCount < 8) {
            ...
            g_usnIoData[tail] = data; g_usnIoSize[tail] = size; g_usnIoCount++;
        }
    }
    EXPORT void SetUsnOverlappedAbort() { g_usnOverlappedAbort = 1; }
    EXPORT void ResetTestState() { g_maxThreads = 0; g_allocFailCountdown = 0; g_readFailCountdown = 0; g_namePoolCapacityOverride = 0; g_failFileSize = 0; g_failPathConversion = 0; g_failPlatformReadCountdown = 0; g_failPlatformWrite = 0; g_usnIoFailError = 0; g_usnIoFailCountdown = 0; g_usnIoHead = 0; g_usnIoCount = 0; g_usnOverlappedAbort = 0; }
#else
    EXPORT void ResetTestState() { g_maxThreads = 0; g_allocFailCountdown = 0; g_readFailCountdown = 0; g_namePoolCapacityOverride = 0; g_failFileSize = 0; g_failPathConversion = 0; g_failPlatformReadCountdown = 0; g_failPlatformWrite = 0; }
#endif
}
