...
extern "C" {
    ...
    // Query the USN journal metadata for the given volume handle.
    // Returns a heap-allocated UsnJournalInfo; caller must free with FreeUsnJournalInfo.
    ...
    // Read USN journal entries starting at startUsn.
    // Returns a heap-allocated UsnJournalResult; caller must free with FreeUsnJournalResult.
    EXPORT UsnJournalResult* ReadUsnJournal(HANDLE volumeHandle, int64_t startUsn, uint64_t journalId) {
        ...
        constexpr size_t readBufferSize = 64 * 1024; // 64KB
        ...
        for (;;) {
            ...
            // First 8 bytes are the next USN cursor
            int64_t bufferNextUsn;
            memcpy(&bufferNextUsn, readBuffer, sizeof(int64_t));

            if (bufferNextUsn == nextUsn) break; // no progress

            nextUsn = bufferNextUsn;

            // Walk variable-length USN_RECORD_V2 entries that follow
