...
static BOOL UsnDeviceIoControl(HANDLE handle, DWORD ioControlCode, LPVOID inBuffer, DWORD inBufferSize,
    ...
    return DeviceIoControl(handle, ioControlCode, inBuffer, inBufferSize, outBuffer, outBufferSize, bytesReturned,
                           overlapped);
}
...
EXPORT UsnJournalInfo* QueryUsnJournal(HANDLE volumeHandle) {
    ...
    if (UsnDeviceIoControl(volumeHandle, FSCTL_QUERY_USN_JOURNAL, nullptr, 0, &journalData, sizeof(journalData),
