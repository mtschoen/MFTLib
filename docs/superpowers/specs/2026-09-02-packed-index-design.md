...
| Field | Type | Notes |
|---|---|---|
| parent row | u32 | row index of the parent; root points at itself |
| name offset | u32 | bytes into the name pool |
| name length | u16 | UTF-16 code units |
| row flags | u16 | in use, directory, tombstone, size unknown |
| attributes | u32 | NTFS file attributes |
| size | u64 | bytes; 0 for directories |
| modified | i64 | UTC ticks |
...
ratio. A rename appends the new name and then swaps offset and length in the
row. A reader sees the old name or the new one, never a torn one. Names are
not interned in v1.
...
- Delete: set tombstone, keep the name so a Recent Changes line can still say
  what was deleted.
- Rename or move: append the new name if changed, swap name offset and length,
...
UTF-16 spans against the pool with NTFS case folding. `DuplicateNames` builds a
transient open-addressing table from name hash to first row and discards it.
