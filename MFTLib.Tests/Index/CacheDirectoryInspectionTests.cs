...
public class CacheDirectoryInspectionTests
{
    ...
    public void InspectCached_HeldCorruptBlockIsInUseUntilReleased()
    {
        ...
        using (var owner = new FileStream(BlockOwnerLock.LockPathFor(path),
        ...
        Assert.AreEqual((BlockValidationResult?)BlockValidationResult.InvalidNameDescriptor,
            invalid.Validation);
    ...
    public void InspectCached_AvailableRootsAreMaterializedAndMappingsReleased()
    {
        ...
        Assert.AreEqual((ProducerKind?)ProducerKind.Enumeration, statuses[0].ProducerKind);
        Assert.AreEqual((ProducerKind?)ProducerKind.Mft, statuses[1].ProducerKind);
        foreach (var status in statuses)
        {
            Assert.AreEqual(CachedBlockAvailability.Available, status.Availability);
            Assert.AreEqual((BlockValidationResult?)BlockValidationResult.Valid, status.Validation);
    ...
    public void InspectCached_RejectionReasonsArePreservedAndListingContinues()
    {
        ...
        for (var index = 0; index < reasons.Length; index++)
        {
            ...
            Assert.AreEqual((BlockValidationResult?)reasons[index], statuses[index].Validation);
    ...
    public void InspectCached_UnrecognizedProducerHasNoRoot()
    {
        ...
        Assert.AreEqual((BlockValidationResult?)BlockValidationResult.Valid, result.Validation);
        Assert.AreEqual((ProducerKind?)((ProducerKind)99), result.ProducerKind);
