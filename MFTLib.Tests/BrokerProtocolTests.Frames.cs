...
public partial class BrokerProtocolTests
{
    ...
    {
        var buffer = new ArrayBufferWriter<byte>();
        BrokerProtocol.WriteScanReady(buffer, "mftlib-scan-123", 8_000_000, 900_000_000, 0);
        var frame = BrokerProtocol.ReadFrame(buffer.WrittenSpan, out _);

        Assert.AreEqual(BrokerFrameKind.ScanReady, frame.Kind);
        Assert.AreEqual("mftlib-scan-123", frame.MmfName);
        Assert.AreEqual(8_000_000, frame.RowCount);
    ...
    {
        AssertWireBytes(w => BrokerProtocol.WriteScanReady(w, "m", 1, 2, 3),
        [
    ...
    {
        var frame = BrokerFrame.ScanReady("mftlib-scan-1", 8_000_000, 900_000_000, 0);
        Assert.AreEqual(BrokerFrameKind.ScanReady, frame.Kind);
